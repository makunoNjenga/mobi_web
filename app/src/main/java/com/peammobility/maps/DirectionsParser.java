        ...
        try {
            ...
            for (int i = 0; i < jRoutes.length(); i++) {
                ...
                for (int j = 0; j < jLegs.length(); j++) {
                    JSONObject data = ((JSONObject) jLegs.get(j));
                    jSteps = data.getJSONArray("steps");
                    ...
                    for (int k = 0; k < jSteps.length(); k++) {
                        ...
                        for (int l = 0; l < list.size(); l++) {
                            ...
                            jDuration = data.getJSONObject("duration");
                            jDistance = data.getJSONObject("distance");

                            hm.put("duration", jDuration.getString("text"));
                            hm.put("distance", jDistance.getString("text"));
                            hm.put("actual_distance", Integer.toString(jDistance.getInt("value")));
